...
mwan3_stop() {
	# 清掉历史遗留的 nft 原生规则（旧版 passwall 用 nft 插入的）
	...
	# 清掉 iptables 插入的规则
	while iptables -t mangle -D mwan3_hook -m connmark --mark ${FWMARK} -j RETURN 2>/dev/null; do :; done
}

mwan3_start() {
	mwan3_stop
	# 必须用 iptables(xtables-nft) 插入，不能用 nft 原生语法：
	# nft 原生表达式会让 iptables-nft 无法读取 mwan3_hook 链，
	# 使 /etc/hotplug.d/iface/15-mwan3 的 `iptables -t mangle -S mwan3_hook` 守卫失败，
	# 所有 ifup/ifdown 事件被静默丢弃 → mwan3 拔插不检测、故障转移失效。
	iptables -t mangle -S mwan3_hook >/dev/null 2>&1 &&
		iptables -t mangle -I mwan3_hook -m connmark --mark ${FWMARK} -j RETURN >/dev/null 2>&1
}
